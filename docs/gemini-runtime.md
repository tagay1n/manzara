...
- Keys are grouped by account and runtime state is stored only in local SQLite per `(account, key, model)`. Choose a random account, then a random usable key, and try alternatives before waiting. Permit at most one request per minute per tuple. Concurrent task processes coordinate through short SQLite transactions in WAL mode.

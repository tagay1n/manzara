...
| FE-05 | Add frontend behavior-focused tests where applicable | PARTIAL | `tests/frontend/test_core.mjs`, `tests/frontend/test_pages.mjs` (page behavior + normalization interaction tests for pagination, stop-all guard, suggestions refresh payload, bulk queue actions, suggestion accept/reject, merge, history undo, cross-tab queue-open transition, and evidence dialog fetch/render) | Coverage now includes key interaction paths on normalization page; multi-step chained flows with canonical creation/editing still need more depth. |

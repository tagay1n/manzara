"""add resumable Library metadata evaluation state
...
def upgrade() -> None:
    ...
    op.execute(
        f"CREATE INDEX idx_library_metadata_evaluation_state_status "
...
def downgrade() -> None:
    op.execute(
        f"DROP TABLE IF EXISTS {_table('library_metadata_evaluation_state')}"
    )

...
- Document-owned PostgreSQL rows use foreign keys with `ON DELETE CASCADE`; cleanup deletes the `document` row and lets the database remove dependent metadata and workflow state atomically. Upstream metadata is the exception: it has no document foreign key, so cleanup must explicitly delete its matching `library_upstream_metadata` row in the same transaction before deleting the document. `document_cleanup_queue` is audit/control state and intentionally survives document deletion.

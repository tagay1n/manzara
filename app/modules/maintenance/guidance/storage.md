...
- Legacy content migration is PostgreSQL-driven and strictly single-worker. It preserves source archive/image keys, disables boto3 transfer threads, rewrites only referenced `ttimg` URLs, removes the complete figure for images confirmed missing from Yandex, checkpoints cutover before source cleanup, and re-verifies Backblaze before deleting Yandex objects. Missing archives and non-404 storage failures remain item failures.
- After the PDF phase, the same task queries PostgreSQL for non-PDF documents whose `content_url` still points to Yandex `ttcontent`. Those archives are downloaded through the local task workspace and migrated with the same verified archive/image copy, URL cutover, and Yandex cleanup checkpoints as PDFs. The phase never scans unrelated Yandex `ttcontent` objects; rows without a matching document URL remain untouched.

...
- Legacy PDF content migration is PostgreSQL-driven and strictly single-worker. It preserves flat archive/image keys, disables boto3 transfer threads, rewrites only referenced `ttimg` URLs, removes the complete figure for images confirmed missing from Yandex, checkpoints cutover before source cleanup, and re-verifies Backblaze before deleting Yandex objects. Missing archives and non-404 storage failures remain item failures.
- After the PDF phase, the same task scans Yandex `ttcontent` directly. An exact archive-key match in Backblaze permits deletion of the Yandex archive and every Yandex `ttimg` key containing its validated MD5; Backblaze image presence is not required for this match. Unmatched archives remain untouched. Archive and image deletions are checkpointed independently.

"""Library publisher insights sourced from metadata schema.org payloads."""
...
def _mentions_cte_sql() -> str:
    return """
        WITH publisher_mentions AS (
            SELECT
                m.md5 AS md5,
                BTRIM(
                    CASE
                        WHEN jsonb_typeof(p.publisher_item) = 'object'
                            THEN COALESCE(
                                p.publisher_item->>'name',
                                p.publisher_item->>'legalName',
                                p.publisher_item->>'alternateName',
                                ''
                            )
                        WHEN jsonb_typeof(p.publisher_item) = 'string'
                            THEN REGEXP_REPLACE(p.publisher_item::text, '^"|"$', '', 'g')
                        ELSE ''
                    END
                ) AS raw_name
            FROM metadata m
            CROSS JOIN LATERAL (
                SELECT elem AS publisher_item
                FROM jsonb_array_elements(
                    CASE
                        WHEN jsonb_typeof((m.schema_org::jsonb)->'publisher') = 'array'
                            THEN (m.schema_org::jsonb)->'publisher'
                        ELSE '[]'::jsonb
                    END
                ) AS elem
                UNION ALL
                SELECT (m.schema_org::jsonb)->'publisher' AS publisher_item
        ...
        mentions AS (
            SELECT
                ...
                END AS script_label,
                (
                    raw_name ~* '(^|\\s)(ооо|зао|ао|пао|ip|llc|ltd|inc|corp|company|press|publisher|publishing|нәшрият|нәшрияты|издательство|типография)(\\s|$)'
                ) AS has_org_marker
            FROM publisher_mentions
            WHERE raw_name <> ''
        )
    """
...
def _row_to_publisher_item(row: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "org_marker_mentions": int(row.get("org_marker_mentions") or 0),
    }


def get_publisher_overview(top_limit: int = 12) -> Dict[str, Any]:
    """Return high-level publisher statistics and top names."""
    try:
            ...
            stats_row = conn.execute(
                text(
                    ...
                    SELECT
                        ...
                        COUNT(DISTINCT md5) AS docs_with_publishers,
                        ...
                        SUM(CASE WHEN has_org_marker THEN 1 ELSE 0 END) AS org_marker_mentions
                    FROM mentions
                    """
                )
            ).mappings().first() or {}
...
def list_publishers(
    *,
    ...
    """Return paginated table of extracted publisher names."""
    page = max(1, int(page))
...
def _ambiguous_reasons(row: Dict[str, Any]) -> list[str]:
    ...
    if int(row.get("org_marker_mentions") or 0) > 0:
        reasons.append("org_marker_form")
    ...
    *,
    ...
    try:
                ...
                variant_clusters.append(
                    {
                        ...
                        "variants": [_row_to_publisher_item(row) for row in items],
                    }
                )
                ...
                text(
                    ...
                    HAVING COUNT(DISTINCT md5) >= 1
                       AND (
                            ...
                            OR SUM(CASE WHEN has_org_marker THEN 1 ELSE 0 END) > 0
                            OR LENGTH(normalized_name) < 5
                            OR normalized_name = ''
                       )
                    ORDER BY docs_count DESC, mentions_count DESC, raw_name ASC
                    LIMIT :limit
        ...
        for row in queue_rows:
            item = _row_to_publisher_item(row)

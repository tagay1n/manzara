...
- Convert legacy DOC/RTF locally first and use the established Google Drive import/export path only when LibreOffice fails or produces an invalid DOCX. Always delete the temporary Drive file. Converter failures are operational outcomes, not proof that the source document is corrupt.

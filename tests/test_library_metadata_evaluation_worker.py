...
def test_worker_marks_rejected_item_terminal_and_continues(
    ...
    def evaluate(doc, **_kwargs):  # noqa: ANN001
        if doc.md5 == first.md5:
            raise GeminiModelPoolItemRejectedError(
                "Gemini request rejected (400): upload terminated"
            )
        return GeminiModelPoolResult(
            model_name="model-first",
            value=Evaluation(applicable=False, reason="not a library document"),
            unavailable_models=(),
        )
    ...
    monkeypatch.setattr(
        ...
        lambda md5, **kwargs: terminal_calls.append((md5, kwargs["reason"])),
    )
    monkeypatch.setattr(
        evaluation_worker_module,
        "save_evaluation_result",
    ...
    assert channel.get_fatal_error() is None
    assert db.progress[-1]["terminal"] == 1
    assert db.progress[-1]["succeeded"] == 1
    assert db.progress[-1]["current"] == 2
    assert db.progress[-1]["remaining"] == 0
